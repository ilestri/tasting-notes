...
export default defineEventHandler(async (event) => {
  ...
  const result = db.transaction((tx) => {
    const newProductId = productId || randomUUID()
    const existingProduct = tx.select().from(products).where(eq(products.id, newProductId)).get()
    ...
    const noteInsert: typeof notes.$inferInsert = {
      id: noteId,
    ...
    tx.insert(notes).values(noteInsert).run()
